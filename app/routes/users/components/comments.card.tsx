...
const CommentsCard = React.memo(({ className, comment, user, ...props }: React.ComponentProps<"article"> & Props) => {
	...
	return (
		<Link to={`/comments/${comment.id}`}>
			<article
				className={cn(
					"flex h-auto max-h-96 min-h-28 w-full flex-col justify-between gap-3 p-4 px-5 hover:bg-sidebar-foreground/10 dark:hover:bg-sidebar-accent/50",
					className,
				)}
			>
				<span className="flex h-full flex-col items-start justify-start gap-2 overflow-hidden text-ellipsis">
